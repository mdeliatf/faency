...
const getStitches = (prefix?: string) =>
  createStitches({
    theme: {
      colors: {
        ...colors,
...
type StitchesInstance = ReturnType<typeof getStitches>;

const defaultStitchesInstance = getStitches();

let stitchesInstance: StitchesInstance | null = null;

function ensureInstance(): StitchesInstance {
  if (!stitchesInstance) {
    return defaultStitchesInstance;
  }
  return stitchesInstance;
}

export function initStitches(prefix?: string) {
  if (stitchesInstance) {
    console.warn('Faency has already been initialized; ignoring re-initialization.');
    return;
  }
  stitchesInstance = getStitches(prefix);
}

export type CSS<T = typeof defaultStitchesInstance.config> = StitchesCSS<T>;

export const styled: typeof defaultStitchesInstance.styled = (...args) =>
  ensureInstance().styled(...args);

export const css: typeof defaultStitchesInstance.css = (...args) => ensureInstance().css(...args);

export const createTheme: typeof defaultStitchesInstance.createTheme = (...args) =>
  ensureInstance().createTheme(...args);

export const getCssText: typeof defaultStitchesInstance.getCssText = () =>
  ensureInstance().getCssText();

export const globalCss: typeof defaultStitchesInstance.globalCss = (...args) =>
  ensureInstance().globalCss(...args);

export const keyframes: typeof defaultStitchesInstance.keyframes = (...args) =>
  ensureInstance().keyframes(...args);

export const config = () => ensureInstance().config; // NOTE: Breaking change!

// @ts-expect-error
export const utils = () => config().utils; // NOTE: Breaking change!
